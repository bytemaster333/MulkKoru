      ...
      renderItem={({ item }) => {
        ...
        return (
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            className="flex-row items-center gap-3 rounded-xl bg-surface-border/30 p-3"
          >
            <View className={`rounded-xl p-2 ${overdue ? 'bg-danger/20' : urgent ? 'bg-warning/20' : 'bg-brand/20'}`}>
              {overdue
            ...
            <View className="flex-1 gap-0.5">
              <Text className="text-sm font-medium text-dark-text" numberOfLines={1}>
            ...
            <Text className={`text-sm font-bold ${overdue ? 'text-danger' : 'text-dark-text'}`}>
